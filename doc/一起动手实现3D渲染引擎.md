...
![图10-24 3D物体和它们的包围球](images/sphere-objects.png)

<center>图10-24 3D物体和它们的包围球</center>
...
<center>图10-26 包围球利用AABB构建法效果</center>
...
![10-27 包围球的Ritter算法](images/sphere-ritter.png)

<center>图10-27 包围球的Ritter算法</center>

<center>图10-28 包围球Ritter算法效果</center>

　　然而，这种算法看起来包围球把物体也包裹得不错了，那有没有更好的包裹着物体并且这个包围球最小呢？答案是有的，下面介绍最后一种算法。

　　最后一种就是Welzl在1991年提出的**最小圆覆盖法**。主要思路是：从顶点集合 S 中任意先选择一个点，作为包围球的初始球心位置 $\vec c$，然后半径初始化 r = 0。

　　这里涉及到3个不共线顶点构成的三角形外接圆，前面讲过了三角形外接圆的圆心坐标和半径，这里就不再讨论了。但是算法里还涉及到4个不共面顶点构成的四面体外接球，所以这里简单讨论下球心坐标和半径的公式。

　　参考前面提到三角形任意一点的重心坐标公式10.26，三角形重心坐标是分成三个子三角形面积和整个三角形面积的比例。这里我们相应的可以得出，空间四面体中任意一点的重心坐标是分成的四个子四面体和整个四面体体积的比例。

　　设 $\vec A$ ， $\vec B$ ， $\vec C$ ， $\vec D$ 分别为四面体四个顶点， $\vec O$ 是四面体内的一点，$V_{O-BCD}$ 为顶点 $\vec O ， \vec B ，\vec C ， \vec D$ 所围成的空间子四面体的体积，$V_{O-ACD}$ 为顶点 $\vec O ，\vec A ，\vec C ，\vec D$ 所围成的空间子四面体的体积，$V_{O-ABD}$ 为顶点 $\vec O ，\vec A ，\vec B ，\vec D$ 所围成的空间子四面体的体积，$V_{O-ABC}$ 为顶点 $\vec O ，\vec A ，\vec B ，\vec C$ 所围成的空间子四面体的体积，$V_{ABCD}$ 为空间四面体的体积。则有等式：
$$
\begin{aligned}
\frac{V_{O-BCD}}{V_{ABCD}} + \frac{V_{O-ACD}}{V_{ABCD}} + \frac{V_{O-ABD}}{V_{ABCD}} + \frac{V_{O-ABC}}{V_{ABCD}} = 1
\end{aligned}
\tag{10.42}
$$
