...
Sphere minSphere(Point P[], int n, Point S[], int s)
{
    ...
    {
        // 当最后一个顶点位于前 n 个顶点构造的最小包围球之外时，放入点集 B，重新构建
        S[s+1] = P[n-1];
        
        // 把最后一个顶点放到最前面，代替从点集 P 中移除的操作
        for (i = n - 2; i = 0; i--)
        {
            Point T = P[i];
            P[i] = P[i + 1];
...
　　这里涉及到3个不共线顶点构成的三角形外接圆，前面讲过了三角形外接圆的圆心坐标和半径，这里就不再讨论了。但是算法里还涉及到4个不共面顶点构成的四面体外接球，所以这里简单讨论下球心坐标和半径的公式。如下图10-29所示的四面体：
