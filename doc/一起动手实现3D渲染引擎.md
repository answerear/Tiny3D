...
　　注意到所有的分子和坟墓中都有 $\vec n$ ，因此，实际上并不必正则化 $\vec n$ 。此时，分母为 $\vec n \cdot \vec n$ 。
...
![图10-19 奔驰定理](images/triangle-benz.png)
...
　　由于重心分中线长之比为2:1，则有 $S_{A} = S_{B} = S_{C} = \frac{1}{3} S_{\triangle}$，根据公式10.29可得：
$$
\begin{aligned}
...
　　由于 $S_{A} = \frac{1}{2}l_1r$ ，$S_{B} = \frac{1}{2}l_2r$ ， $S_C = \frac{1}{2}l_2r$ ， 则有 $S_{\triangle} = \frac{1}{2}(l_1+l_2+l_3)r$ ，设 $p = l_1 + l_2 + l_3$ 是三角形的周长，代入公式10.29，有：
$$
\begin{aligned}
...
\end{aligned}
$$
　　有了这些临时变量，可以得到外心的重心坐标为：
$$
(\frac{c_2 + c_3}{2c}, \frac{c_3 + c_1}{2c}, \frac{c_1 + c_2}{2c})
$$
　　外心为：
$$
\vec c_{circ} = \frac{(c_2 + c_3)\vec v_1 + (c_3 + c_1) \vec v_2 + (c_1 + c_2) \vec v_3}{2c}
$$
　　外接圆半径为：
$$
r_{circ} = \frac{\sqrt{(d_1 + d_2)(d_２ + d_3)(d_3 + d_1)/c}}{2}
$$
　　外心和外接圆半径解决了寻找过三个点的圆的问题。

## 10.6 包围盒

　　我们有很多时候需要用一些物体来界定物体，这里面我们最基本的选择有包围球（Sphere Bound）、轴对齐包围盒（Axially Aligned Bounding Box）和有向包围球（Oriented Bounding Box）。当然有些高级引擎还有更多种选择，例如：胶囊体、圆柱体、圆锥体等等一些更加复杂的包围盒，我们这里仅仅讨论三种简单的。包围盒有什么用？主要是用来做物体相交性检测？相交性检测有什么用？主要就是做一些碰撞检测、物体剔除等功能。这些后面会讨论到。

### 10.6.1 包围球 （Sphere）

#### 10.6.1.1 包围球的表示

　　在2D中，一种常见的表示物体边界的几何图元是包围圆。相对应的，在3D世界中，这种表示物体边界的几何图元就是包围球。包围圆和包围球就是用一个圆或者球体代替物体轮廓来界定整个物体。为什么用包围球呢？因为球体缩放和旋转变换不会改变它的形状，相对来说计算量大大减低。在前面我们就已经讨论过了圆和球，这里就不详细讨论了。需要注意的是球心的选取的问题。球心选取合适，那么半径可以比较小，同时这个球能够较好的包裹着物体，在代替物体轮廓上可以相对精确一些。但是无论怎样，球体在许多情况下都表示得很粗略。如下图10-23所展示的一些简单3D物体和它们的包围球：

![图10-23 3D物体和它们的包围球](G:/Workspace/MyGame/Tiny3D/steps/doc/images/sphere-objects.png)

<center>图10-23 3D物体和它们的包围球</center>
...
　　这样有了球心点和半径后，我们就能唯一确定一个包围球了。

#### 10.6.1.2 包围球的构建

　　要求一个包围球，按照上面的表示法，我们就要求出球心和半径，这样子才能唯一确定一个包围球。包围球的构建算法有许多，这里我们介绍四种常见算法。

　　第一种是**均值法**。下面讲述下原理。在3D世界中，模型一般都是用一系列的顶点来描述，所以，我们就利用这些模型顶点来构建一个跟模型轮廓相对贴近的包围球。那怎么求球心和半径呢？这里我们简单的通过把所有模型顶点相加，然后除以顶点数，来近似作为球心位置。然后把模型所有顶点逐个遍历，计算每一个顶点与球心的距离，选取最长的距离作为包围球的半径。这样子，我们就有了球心和半径。因为这个方法是直接取所有顶点的位置均值来作为球心，所以就称为均值法。但是这种方法很粗糙，表示出来的包围球有许多体积都是空白浪费掉的，所以我们引入后面的几种算法来改进。

<center>图10-24 包围球均值法效果</center>

　　第二种是**快速的恒时单通路算法**。在这里，我们借助AABB来构建。我们用这个已经构造好的AABB中心和对角线来形成球体。当然，这种方法有时候不一定能满足需求，这里可以进一步优化。具体就是，从AABB的中心出发，将它作为包围球的中心，然后再次遍历所有的顶点，找出距离球心最远的顶点，把这个最远距离作为包围球的新半径。这里为了避免平方根计算损耗，我们直接比较他们的平方值。

<center>图10-25 包围球利用AABB构建法效果</center>

　　第三种叫**Ritter算法**，是Ritter Jack提出来的一种近似计算包围球的改进算法。主要思路是：分别找到这个模型在x，y，z正负留个方向上的最远距离的六个点，然后分别计算出这三对点之间的长度，也就是x轴向上两个点之间的长度，y轴向上两个点之间的长度，z轴向上两个点之间的长度。我们选取长度最长的那个一个作为包围球的半径，以这个长度的两个点的中点作为包围球的球心。接着遍历模型中的每一个顶点，检查顶点到球心之间的距离d，如果这个顶点在包围球半径内，直接跳过忽略它，如果不在里面，我们按照以下算法对包围球修正，以使新的包围球能包含这个点。

<center>图10-26 包围球的Ritter算法</center>

　　我们假设当前的包围球心为O，半径为r。假设包围球外一点P。按照上面说法，我们要对这个包围球进行修正，需要把这个点P包围在球体里面。为了最紧凑的包围住这个点，我们将点P与球心O连线，交圆于T点。这样，TP就是我们要求的包围球的新直径了，球心也就是他们之间的中点。这是，新的半径是(r + OP) * 0.5。计算出新半径后，最后就是平移老的球心点到新的位置，平移的向量大小刚好就是SP的长度，方向是从O点指向P点，其中S点为PR的中点。

　　按照上面的方式，不断遍历所有顶点，最后就能确定一个近似的包围球。

<center>图10-27 包围球Ritter算法效果</center>

　　最后一种Welzl提出的**最小圆覆盖法**。主要思路是：从模型顶点集合中任意先选择一个点，作为包围球的初始球心位置，然后半径初始化为0。
...
　　另外，我们还需要变换半径，因为物体会缩放，半径只受物体的缩放影响。那物体有三个方向的缩放值，应该取哪个一个呢？不管物体是球体还是非球体物体 ，都取三个轴上最大的缩放值来缩放半径，以确保包围球能够按照原来的样子包裹住物体。假设球体半径为 r，三个轴的缩放值为 $s_x$，$s_y$，$s_z$，则变换如下：
$$
\begin{aligned}
& s = max\{s_x, s_y, s_z\} \\
& r^{'} = sr
...
　　另一种常见的用来表示物体边界的几何图元就是矩形包围盒。在3D中就是六面体包围盒。当然矩形或者六面体包围盒可以是与轴对齐的，也可以是任意方向的。轴对齐包围盒有一个限制，那就是它的边必须垂直于坐标轴。经常用缩写AABB(Axially Aligned Bounding Box）来表示轴对齐矩形（六面体）包围盒。后面还会介绍另外一种矩形（六面体）包围盒。轴对齐矩形（六面体）包围盒不仅容易创建，而且易于使用，接下来我们来详细讨论下AABB。

　　一个3D的AABB就是一个简单的六面体，每一边都平行于一个坐标平面。六面体包围盒不一定都是立方体，它的长、宽、高可以彼此不同。在下图10-23中展示了一些简单的3D物体的它们的AABB。

<center>图10-23 3D物体和它们的AABB</center>
...
\end{aligned}
$$
　　明确地定义一个AABB只需要 $\vec p_{min}$ ，$\vec p_{max}$ ，$\vec c$ ， $\vec s$ ， $\vec r$ 这5个向量中的两个（除 $\vec s$ 和 $\vec r$ 不能配对外，它们中的任意两个都可配对）。在一些情况下，某些配对形式逼其他的会更有用。我们建议用 $\vec p_{min}$ 和 $\vec p_{max}$ 表示一个AABB，因为实际应用中，使用它们的频率远高于 $\vec c$ ，$\vec s$ ，$\vec r$ 。当然，由 $\vec p_{min}$ 和 $\vec p_{max}$ 计算其余三个中的任意一个都是很容易的。

　　计算一个顶点集合的AABB是非常简单的：先将最小值和最大值设为“ 正负无穷大”或任何逼实际中用到的数都大或者小得多的数。接着，遍历全部顶点，并扩展AABB知道它包含所有点为止。

　　之前我们在讨论包围球作为界定体的时候，就说过包围球在很多情况下精度都不粗，所以，在很多情况下，AABB比包围球更适合于做界定体。

- 计算一个顶点集的AABB，在编程上更容易实现，并能在短时间内完成。计算包围球则复杂得多了。
- 对实际世界里的许多物体，AABB提供了一种“更紧凑”的界定体。当然，对于某些物体，包围球更好（例如足球、篮球、桌球等球形物体）。在极端的情况下，AABB的体积可能仅相当于包围球二分之一。大部分时候包围球的体积会比AABB体积大得多。比较一个长方体的包围球和AABB就知道了，如下图10-23所示：

<center>图10-23 长方体的包围球和AABB</center>

　　包围球的根本问题是它的形状只有一个自由度——半径。而AABB却有三个自由度——长、宽、高。因此，它可以调节这些自由度以适应不同物体。通过上图10-23，我们可以注意到AABB对物体的方向是敏感的。比较图中两个长方体，它们大小相同，只是方向不同而已；而包围球在这两种相同大小不同方向下的长方体，大小是相同的。因此包围球对物体方向是不敏感的。

　　当物体在虚拟世界中移动时，它对应的AABB也需要随之移动。此时，我们有两个选择——用变换后的物体来重新计算AABB，或者对AABB做和物体同样的变换。所得到的结果不一定是轴对齐的（如果物体旋转）也不一定是盒装的（如果物体发生了扭曲）。不过，通过变换后的AABB进行计算要比通过经过变换的物体计算AABB快得多，因为AABB只有8个顶点。
...
　　根据变换的不同，这种方法可能使新AABB比原来大许多。例如，在2D中，45°的旋转会大大增加AABB的尺寸。如下图10-24所示：

<center>图10-24 变换后的AABB</center>

　　比较土10-24中原AABB（左边）和新AABB（右边较大的框），它是通过旋转后的AABB计算的，新AABB几乎是原来的两倍。注意，如果从旋转后的物体而不是通过旋转后的AABB来计算新AABB，它的大小将和原来的AABB相同。
