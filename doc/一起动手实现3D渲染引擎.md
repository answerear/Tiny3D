...
　　我们有很多时候需要用一些物体来界定物体，这里面我们最基本的选择有包围球（Sphere Bound）、轴对齐包围盒（Axially Aligned Bounding Box）和定向包围球（Oriented Bounding Box）。当然有些高级引擎还有更多种选择，例如：胶囊体、圆柱体、圆锥体等等一些更加复杂的包围盒，我们这里仅仅讨论三种简单的。包围盒有什么用？主要是用来做物体相交性检测？相交性检测有什么用？主要就是做一些碰撞检测、物体剔除等功能。这些后面会讨论到。
