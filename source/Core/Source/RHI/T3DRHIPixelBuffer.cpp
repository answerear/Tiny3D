...
namespace Tiny3D
{
    ...
    RHIPixelBuffer::RHIPixelBuffer(size_t width, size_t height,
    ...
    void *RHIPixelBuffer::lock(const Rect &rect, LockOptions options, 
        size_t &lockedPitch)
    {
        T3D_ASSERT(!isLocked(), "RHIPixelBuffer already locked !");
    ...
    void *RHIPixelBuffer::lock(LockOptions options)
    {
    ...
    TResult RHIPixelBuffer::copyTo(RHIPixelBufferPtr dst, 
        Rect *dstRect/* = nullptr*/, Rect *srcRect/* = nullptr*/)
    {
        ...
        do
        {
            ...
            dstData = (uint8_t *)dst->lock(LockOptions::kWrite);
            if (dstData == nullptr)
            {
            ...
            srcData = (uint8_t *)lock(LockOptions::kRead);
            if (srcData == nullptr)
            {
                T3D_LOG_ERROR(LOG_TAG_RENDER,
    ...
    size_t RHIPixelBuffer::readData(size_t offset, size_t size, void *dst)
    {
    ...
    size_t RHIPixelBuffer::writeData(size_t offset, size_t size, 
    ...
    void *RHIPixelBuffer::lockImpl(size_t offset, size_t size, 
        LockOptions options)
    {
