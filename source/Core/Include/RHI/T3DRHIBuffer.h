...
namespace Tiny3D
{
    ...
    public:
        ...
        {
            kCPUNone = 0,           /**< CPU 不访问 */
            kCPUWrite = (1 << 0),   /**< CPU寫數據，GPU讀數據 */
            kCPURead = (1 << 1),    /**< CPU讀數據，GPU寫數據 */
            kGPUCopy = (1 << 2)     /**< 從GPU讀數據，作為另一个GPU操作數據源 */
        };
        ...
        {
            kRead = 0,          /**< 映射到内存的资源用于读取 */
            kWrite,             /**< 映射到内存的资源用于写入 */
            kReadWrite,         /**< 映射到内存的资源用于读写 */
            kWriteDiscard,      /**< 映射到内存的资源用于写入，之前的资源数据将会被抛弃 */
            kWriteNoOverwrite   /**< 映射到内存的资源用于写入，但不能复写已经存在的资源 */
        };
         ...
         * @remarks 具体的渲染系统子类需要实现该接口.
         */
        virtual size_t writeData(size_t offset, size_t size, const void *src, bool discardWholeBuffer = false) = 0;
         ...
         * @returns 返回复制的字节数.
         */
        virtual size_t copyData(RHIBufferPtr srcBuffer, size_t srcOffset, size_t dstOffset, size_t size, bool discardWholeBuffer = false);
         ...
         * @returns 返回复制的字节数.
         */
        virtual size_t copyData(RHIBufferPtr srcBuffer);
