...
namespace Tiny3D
{
    ...
    static unsigned int pos_of_char(const unsigned char chr)
    {
        ...
        else if (chr >= '0' && chr <= '9') return chr - '0' + ('Z' - 'A') + ('z'
            - 'a') + 2;
        ...
        while (pos < in_len)
        {
            ...
            if (pos + 1 < in_len)
            {
                ...
                if (pos + 2 < in_len)
                {
                    ...
                    ret.push_back(
                        base64_chars_[bytes_to_encode[pos + 2] & 0x3f]);
                }
                else
                {
                    ret.push_back(
                              ...
                              bool remove_linebreaks)
    {
        ...
        while (pos < length_of_string)
        {
            ...
            )
            {
                ...
                //
                unsigned int pos_of_char_2 = pos_of_char(
                    encoded_string.at(pos + 2));
                ret.push_back(
                    static_cast<String::value_type>(((pos_of_char_1 & 0x0f)
                        << 4) + ((pos_of_char_2 & 0x3c) >> 2)));
                ...
                )
                {
                    ...
                    //
                    ret.push_back(
                        static_cast<String::value_type>(((pos_of_char_2 &
                            0x03) << 6) + pos_of_char(
                            encoded_string.at(pos + 3))));
                }
            }
    ...
    //
    // Interface with std::string_view rather than const String&
    // Requires C++17
    // Provided by Yannic Bonenberger (https://github.com/Yannic)
    //

    String base64_encode(std::string_view s, bool url) {
       return encode(s, url);
    }

    String base64_encode_pem(std::string_view s) {
       return encode_pem(s);
    }

    String base64_encode_mime(std::string_view s) {
       return encode_mime(s);
    }

    String base64_decode(std::string_view s, bool remove_linebreaks) {
       return decode(s, remove_linebreaks);
    }
