...
![图4-1 日志系统类图](doc/images/s02-logsystem-design.png)
...
　　介绍完设计目的，我们先看看基于流式数据的IO类结构和设计，如下图4-3：![图4-3 流式数据类图](doc/images/s02-stream-design.png)
...
![图6-1 2D笛卡尔坐标系](doc/images/2D-cartesian.png)
...
![图6-2 屏幕坐标系](doc/images/screen-cartesian.png)
...
![图6-3 2D笛卡尔坐标系中定位点](doc/images/2D-cartesian-position.png)
...
![图6-4 3D笛卡尔坐标系](doc/images/3D-cartesian.png)
...
![图6-5 3D笛卡尔坐标系中定位点](doc/images/3D-cartesian-position.png)
...
![图6-6 3D左手坐标系](doc/images/left-hand-cartesian.png)

<center>图6-6 3D左手坐标系</center>

　　右手坐标系，如下图6-7所示：

![图6-7 3D右手坐标系](doc/images/right-hand-cartesian.png)
...
![图6-8 相机坐标系](doc/images/3d-camera-cartesian.png)
...
![图7-3 2D向量的表达方式](doc/images/vector2D-presentation.png)
...
![图7-4 将向量表示为位移序列](doc/images/vector3D-translate.png)

<center>图7-4 将向量表示为位移序列]</center>

　　这些步骤的执行顺序无关紧要。比如，可以先向前移动3个单位，再向上移动2个单位，最后向右移动1个单位。仍然可以达到同样的位移。不同的顺序对应着向量轴对齐包围盒上的不同路径。后面将从数学意义上验证这个几何直观现象。

## 7.2 向量与点的关系

　　“点”有位置，但没有实际的大小。之前我们说过向量时有大小和方向，但没有位置。所以使用“点”和“向量”的目的完全不同。“点”描述位置，而“向量”描述位移。如下图7-5比较了两幅图。

![图7-5 指明点与指明向量](doc/images/vector-point.png)
...
- 讨论平面的表示
- 讨论三角形的表示
- 讨论包围球（Sphere）、轴对齐包围盒（AABB）和定向包围盒（OBB）的表示
- 讨论视锥体的表示
...
![图10-1 参数形式的圆](doc/images/circle-parameters.png)
...
![图10-2 直线斜截式几何表示](doc/images/line-intercept.png)
...
![图10-3 用垂直向量和到原点的距离定义直线](doc/images/line-distance.png)
...
![图10-4 用垂直向量和直线上的一点来定义直线](doc/images/line-direction.png)

<center>图10-4 用垂直向量 $\vec n$ 和直线上的一点来定义直线</center>

　　第六种表示方式，也是最后一种表示方式为，将直线作为两个点 q 和 r 的垂直平分线，如下图10-5所示：

![图10-5 用线段的垂直平分线定义直线](doc/images/line-2pts-distance.png)
...
![图10-6 射线定义](doc/images/ray-definition.png)
...
![图10-3 射线的参数形式](doc/images/ray-parameter.png)
...
![图10-8 球的定义](doc/images/shpere.png)
...
![图10-9 平面的正面和反面](doc/images/plane.png)
...
![图10-10 三个点定义平面](doc/images/plane-3pts.png)
...
![图10-11 计算点到平面的距离](doc/images/plane-pts-distance.png)
...
![图10-12 三角形定义](doc/images/triangle.png)
...
![图10-13 平行四边形中的三角形](doc/images/parallelogram.png)
...
　　先在2D中解决这个问题。基本思想是，对三角形三边中的每一边，计算上由该边，下由 x 轴所围成的梯形的有符号面积，如下图12-14所示：

![图10-14 向量围的面积](doc/images/triangle-area.png)
...
　　这里，假设顶点是按照顺时针列出来的，如果顶点以相反的顺序列出来，面积的符号将变负。将这三个梯形的面积相加，计算三角形的有符号面积：
$$
\begin{aligned}
A &= A(e_1) + A(e_2) + A(e_3)
\\
...
![图10-15 重心坐标示例](doc/images/triangle-centroid.png)
...
第三，不只是三角形内的点，该平面上的所有点都能用重心坐标描述。三角形内的点的重心坐标在范围0到1之间变化。三角形外的点至少有一个坐标为负。重心坐标用和原三角形大小相同的块“嵌满”整个平面。如下图10-16所示：

<center>图10-16 重心坐标嵌满整个平面</center>
